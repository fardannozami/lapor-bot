...
function App() {
  ...
  const [page, setPage] = useState<Page>(() => {
    // AuthProvider loads the persisted session synchronously from
    // localStorage on first render, so on a page refresh we can decide
    // the initial route right away — keeping the user on their profile
    // instead of bouncing back to the leaderboard.
    if (authUser && token) {
      const isPhoneLike = !authUser.name || /^[\d+]/.test(authUser.name);
      return isPhoneLike ? "profile-setup" : "personal";
    }
    return "dashboard";
  });
  const [personalUser, setPersonalUser] = useState<EnrichedReport | null>(null);

  // Sync personalUser with auth state on token change (login / logout).
  useEffect(() => {
    if (authUser && token) {
      ...
      <header className="max-w-7xl mx-auto pt-8 pb-6 border-b border-gray-850 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-xs text-gray-500 font-mono mt-1 tracking-wider uppercase">
            Healthy with sports consistency leaderboard — Where consistency
            becomes rank
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto justify-between md:justify-end">
          ...
          {/* Account / profile action — visually separated, placed at the
						trailing edge so users always know where their profile lives. */}
          <div
            className="hidden md:block w-px h-6 bg-gray-800 mx-1"
            aria-hidden="true"
          />
          ...
          {personalUser ? (
            <button
              ...
              aria-label={`Buka profil ${personalUser.name || "saya"}`}
            >
              <UserCircle2 size={16} />
      ...
      <main className="max-w-7xl mx-auto">
        ...
        {page === "personal" && personalUser && (
          <PersonalPage
            ...
            onUserRefresh={(refreshed) => setPersonalUser(refreshed)}
          />
        )}
          ...
          <>
            ...
            {error && (
              <div className="mb-6 p-4.5 rounded-2xl bg-system-red/10 border border-system-red/35 flex items-start gap-3 text-sm text-red-300">
                <AlertCircle
                  className="text-system-red mt-0.5 shrink-0"
                  size={18}
                />
                <div>
            ...
            {loading ? (
              ...
              <div className="glass rounded-3xl p-16 text-center border border-gray-800">
                <ShieldAlert
                  className="text-system-gold mx-auto mb-4 animate-bounce"
                  size={40}
                />
                ...
                <p className="text-xs text-gray-500 font-mono mt-2 max-w-sm mx-auto">
                  The database does not contain any active reports yet. Have
                  members submit a workout report using "/lapor" on WhatsApp!
                </p>
              </div>
            ) : viewMode === "table" ? (
              <LeaderboardTable
                hunters={hunters}
                onSelectHunter={setSelectedHunter}
              />
            ) : (
              <>
                ...
                <nav
                  className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-3 glass rounded-2xl p-4"
                  aria-label="Card pagination"
                >
                  <p className="text-xs text-gray-500 font-mono uppercase tracking-wider">
                    Showing{" "}
                    {visibleCardHunters.length === 0
                      ? 0
                      : (safeCardPage - 1) * PAGE_SIZE + 1}
                    -{Math.min(safeCardPage * PAGE_SIZE, hunters.length)} of{" "}
                    {hunters.length}
                  </p>
